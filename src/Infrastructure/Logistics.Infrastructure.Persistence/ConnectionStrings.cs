namespace Logistics.Infrastructure.Persistence;

/// <summary>
///     Local connection strings used for development (e.g., creating migrations).
/// </summary>
internal static class ConnectionStrings
{
    public const string LocalMaster =
        "Host=localhost; Port=5432; Database=master_dispatchload; Username=postgres; Password=Test12345#";

    public const string LocalDefaultTenant =
        "Host=localhost; Port=5432; Database=default_dispatchload; Username=postgres; Password=Test12345#";
}
